...
export default {
  ...
  actions: {
    ...
    async fetchSchedule(store, force = false) {
      ...
      && !e.wk_holiday.startsWith('WEEKEND_WORKING_OFF')) || (e.wk_holiday.startsWith('HOLIDAY_WORKING_OFF') && ((day) => (day > 0 && day < 6))(new Date(e.wk_date).getDay()))).map((item) => processor(item, expectedDate)));
    },
    record(store, type) {
      const auth = store.rootGetters['user/auth'];
      if (!auth) return;
